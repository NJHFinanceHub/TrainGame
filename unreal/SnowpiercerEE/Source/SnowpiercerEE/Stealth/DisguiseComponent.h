...
class SNOWPIERCEREE_API UDisguiseComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintPure, Category = "Stealth|Disguise")
	float GetDetectionModifier(EStealthTrainZone CurrentZone) const;

	/** Check if the current disguise matches a zone */
	UFUNCTION(BlueprintPure, Category = "Stealth|Disguise")
	bool MatchesZone(EStealthTrainZone Zone) const;
